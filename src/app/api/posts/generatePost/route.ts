    ...
    try {
      ...
      const _generatedTitle = await openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Write me a title for a blog post about ${description}. The keywords for the post are as follow: ${keywords}. The tone of the post are as follow: ${tone}. The title should be SEO friendly and no longer than 15 words. Write only one title. ${
              title.length > 0
