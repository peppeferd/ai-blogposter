...
export default function Sidebar() {
  ...
  return user ? (
    <div className="bg-white border border-gray-100 py-2 flex flex-shrink-0 flew-row justify-around md:justify-start md:flex-col md:h-full md:w-32 z-10">
      {menu.map((item, index) => (
        <Link
          href={item.route}
          key={index}
        ...
        >
          <div className="flex flex-row items-center">
            ...
            <span className="ml-2 text-gray-500 hover:text-indigo-600">
              {item.text}
            </span>
          </div>
        </Link>
